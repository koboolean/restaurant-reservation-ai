...
public class BookingDTOs {
    ...
    public record CreateReservationRequest(
            ...
            String allergies,
            @JsonPropertyDescription("기념일, 생일 등 고객의 특별 요청(없으면 null)")
    ...
    public record OrderItemRequest(
            @JsonPropertyDescription("메뉴 ID")
            Long menuId,
            @JsonPropertyDescription("수량")
            int quantity,
            @JsonPropertyDescription("메뉴별 요청사항(예 : 굽기정도, 옵션 등)")
            String request
    ){}
    ...
    public record AddOrderRequest(
            @JsonPropertyDescription("메뉴를 추가할 예약 ID")
            Long reservationId,
            @JsonPropertyDescription("추가할 메뉴 목록")
            List<OrderItemRequest> orderItems
    ){}

    public record OrderHistoryRequest(@JsonPropertyDescription("주문 내역을 조회할 예약번호(ID)") Long reservationId){}

    public record CancelOrderRequest(@JsonPropertyDescription("주문을 취소할 예약번호(ID)") Long reservationId){}

    public record CancelMenuItemRequest(
            @JsonPropertyDescription("주문을 취소할 예약번호(ID)") Long reservationId,
